...
function forum_admin_showcats($id, $stufe){
	$erg = db_query("SELECT * FROM prefix_forumcats WHERE cid = ".$id." ORDER BY pos");
...
switch($um){
	...
	case 'newForum':
		if(empty($_POST['sub'])){
			...
			db_query("INSERT INTO prefix_forums (cid, view, start, reply, pos, name, besch, erwrecht) 
				VALUES (".$cid.", ".$view.", ".$start.", ".$reply.", ".$a.", '".$name."', '".$text."', $erweitert)");
	...
	##################################################################################
	## - ein bestehendes Forum wird verändert - 									##
	##################################################################################
	case 'changeForum':
		if(empty($_POST['sub'])){
			$chrecht = 0;
			...
			$ar['checkerw'] = '';
			$erg_recht = db_query("SELECT * FROM prefix_forumrecht WHERE recht_fkforum = $fid");
			if(db_num_rows($erg_recht) > 0 AND $chrecht != 1){
				$erg_view = db_query("SELECT * FROM prefix_forumrecht WHERE recht_fkforum = $fid AND recht_bearbeiten = 1");
				$erg_reply = db_query("SELECT * FROM prefix_forumrecht WHERE recht_fkforum = $fid AND recht_bearbeiten = 2");
				$erg_start = db_query("SELECT * FROM prefix_forumrecht WHERE recht_fkforum = $fid AND recht_bearbeiten = 3");
				while($rowr = db_fetch_assoc($erg_view)){
					$arview .= $rowr['recht_fkrecht']."\n";
				}
				while($rowr = db_fetch_assoc($erg_reply)){
					$arreply .= $rowr['recht_fkrecht']."\n";
				}
				while($rowr = db_fetch_assoc($erg_start)){
					$arstart .= $rowr['recht_fkrecht']."\n";
				}
				$ar['m_views'] = $arview;
				$ar['m_reply'] = $arreply;
				$ar['m_start'] = $arstart;
			...
			db_query("UPDATE prefix_forums SET name = '".$name."', besch = '".$text."', cid = ".$cid.", pos = ".$a.", start = ".$start.", reply = ".$reply.", view = ".$view.", erwrecht = $erweitert WHERE id = ".$fid);
		
			if($erweitert == 1){
				$erweitert = 1;				
				db_query("DELETE FROM prefix_forumrecht WHERE recht_fkforum = $fid");				
				$teile = explode("\n", escape($_POST['t_m_views'], 'string'));
				$teilcount = count($teile) - 1;
				while($teilcount > 0){
					$teilcount--;
					$view = $teile[$teilcount];
					db_query("INSERT INTO prefix_forumrecht (recht_fkforum, recht_fkrecht, recht_bearbeiten) VALUES ($fid, $view, 1)");
				}
				...
				while($teilcount > 0){
					$teilcount--;
					$reply = $teile[$teilcount];
					db_query("INSERT INTO prefix_forumrecht (recht_fkforum, recht_fkrecht, recht_bearbeiten) VALUES ($fid, $reply, 2)");
				}
				...
				while($teilcount > 0){
					$teilcount--;
					$start = $teile[$teilcount];
					db_query("INSERT INTO prefix_forumrecht (recht_fkforum, recht_fkrecht, recht_bearbeiten) VALUES ($fid, $start, 3)");
				}
			}		
		}
	break;
	...
	##################################################################################
	## - eine bestähende Kategorie verändern -										##
	##################################################################################
...
if($show){
	...
	while($row = db_fetch_assoc($erg)){
		...
		$erg1 = db_query(
			"SELECT prefix_forums.id as fid,
